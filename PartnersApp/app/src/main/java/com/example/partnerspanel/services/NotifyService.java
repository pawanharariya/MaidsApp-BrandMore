...
//                        Toast.makeText(getApplicationContext(), "firebase:" + firebaseChanges + " changes " + changes, Toast.LENGTH_SHORT).show();
        ...
        colRef2.whereEqualTo("location", pincode)
                .whereEqualTo("confirmStatus", false)
                .addSnapshotListener(new EventListener<QuerySnapshot>() {
                    @Override
                    public void onEvent(@Nullable QuerySnapshot value,
                        ...
                        int changes = sharedPreferences.getInt("changes2", 0);
                        if (firebaseChanges > changes) {
                            editor.putInt("changes2", firebaseChanges);
                            editor.commit();
                            notificationNumber += firebaseChanges - changes;
                            type = 0;
                            onStart(intents, 0, 0);
        ...
        colRef3.whereEqualTo("activeStatus", true)
                .addSnapshotListener(new EventListener<QuerySnapshot>() {
                    ...
                    public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException e) {
                        ...
                        Log.d("notifications", "firebase" + changes);
                        if (changes > seen) {
                            editor.putInt("seen", changes);
                            editor.commit();
                            type = 1;
                            Log.d("notifications", "seen " + seen);
                            onStart(intents, 0, 0);
                        } else {
                            editor.putInt("seen", changes);
                            editor.commit();
